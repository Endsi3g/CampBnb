  ...
  static Future<Map<String, dynamic>> validateCache() async {
    ...
    try {
      ...
      try {
        ...
        await _cacheService.cacheListings(testListings, searchKey: 'test-search');
      ...
      try {
        final cached = _cacheService.getCachedListings(searchKey: 'test-search');
      ...
      // Test 7: Suppression
      try {
        await _cacheService.removeFromCache('cache-test-listing', type: CacheType.listing);
      ...
      results['success_rate'] = (passedTests / totalTests * 100).toStringAsFixed(1);

      if (passedTests == totalTests) {
        _logger.i('✅ Tous les tests du cache ont réussi ($passedTests/$totalTests)');
      } else {
      ...
      _logger.e('❌ Erreur générale lors de la validation', error: e, stackTrace: stackTrace);
    }
