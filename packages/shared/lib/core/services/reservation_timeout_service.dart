...
class ReservationTimeoutService {
  ...
  factory ReservationTimeoutService() => _instance ??= ReservationTimeoutService._internal();
  ...
  Future<void> _checkExpiredReservations() async {
    try {
      ...
      if (response.status == 200) {
        ...
        if (processed > 0) {
          _logger.i('$processed réservation(s) expirée(s) annulée(s) automatiquement');
        ...
        _logger.w('Erreur lors de la vérification des timeouts: ${response.status}');
      }
    } catch (e, stackTrace) {
      _logger.e('Erreur lors de la vérification des réservations expirées', error: e, stackTrace: stackTrace);
  ...
  Future<Map<String, dynamic>> checkExpiredReservationsManually() async {
    try {
      ...
      _logger.e('Erreur lors de la vérification manuelle', error: e, stackTrace: stackTrace);
      ErrorMonitoringService().captureException(
        e,
        stackTrace: stackTrace,
        context: {
