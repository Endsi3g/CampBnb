...
class MFAService {
  ...
  Future<Map<String, dynamic>> enableTOTP() async {
    try {
      ...
      final otpUrl = TOTPHelper.generateOTPAuthUrl(
        secret: secret,
      ...
      return {'secret': secret, 'qrCodeUrl': otpUrl, 'manualEntryKey': secret};
    } catch (e) {
  ...
  Future<bool> verifyTOTP(String code) async {
    try {
      ...
      ).select('mfa_secret').eq('id', user.id).single();
      ...
      // Vérifier le code
      final isValid = TOTPHelper.verifyCode(code, secret);
  ...
  Future<List<String>> generateBackupCodes() async {
    try {
      ...
      await SupabaseService.from(
        'user_security_settings',
      ).update({'backup_codes': codes}).eq('id', user.id);

      return codes;
    } catch (e) {
      AppConfig.logger.e(
        'Erreur lors de la génération des codes de secours: $e',
      );
      rethrow;
    }
  }
  ...
  Future<bool> verifyBackupCode(String code) async {
    try {
      ...
      final response = await SupabaseService.from(
        'user_security_settings',
      ).select('backup_codes').eq('id', user.id).single();

      final backupCodes =
          (response['backup_codes'] as List<dynamic>?)
              ?.map((e) => e.toString())
              .toList() ??
          [];
      ...
      if (backupCodes.contains(code)) {
        ...
        await SupabaseService.from(
          'user_security_settings',
        ).update({'backup_codes': backupCodes}).eq('id', user.id);
      ...
      AppConfig.logger.e(
        'Erreur lors de la vérification du code de secours: $e',
      );
      return false;
    }
  }
  ...
  Future<bool> isMFAEnabled() async {
    try {
      ...
      final response = await SupabaseService.from(
        'user_security_settings',
      ).select('mfa_enabled').eq('id', user.id).maybeSingle();
