...
class GDPRService {
  ...
  Future<void> recordConsent({
    ...
    try {
      ...
      AppConfig.logger.e(
        'Erreur lors de l\'enregistrement du consentement: $e',
      );
      rethrow;
    }
  }
  ...
  Future<Map<String, bool>> getUserConsents() async {
    try {
      ...
      AppConfig.logger.e(
        'Erreur lors de la récupération des consentements: $e',
      );
      return {};
    }
  }
  ...
  Future<Map<String, dynamic>> exportUserData() async {
    try {
      ...
      final listings = await SupabaseService.from(
        'listings',
      ).select().eq('host_id', user.id);

      final reservations = await SupabaseService.from(
        'reservations',
      ).select().or('guest_id.eq.${user.id},host_id.eq.${user.id}');

      final messages = await SupabaseService.from(
        'messages',
      ).select().or('sender_id.eq.${user.id},recipient_id.eq.${user.id}');

      final reviews = await SupabaseService.from(
        'reviews',
      ).select().or('reviewer_id.eq.${user.id},reviewee_id.eq.${user.id}');

      final consents = await SupabaseService.from(
        'gdpr_consents',
  ...
  Future<bool> hasDataProcessingConsent() async {
    try {
      ...
      ).select('gdpr_consent_given').eq('id', user.id).maybeSingle();
